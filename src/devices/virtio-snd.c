...
#define VSND_GEN_TX_QUEUE_HANDLER(NAME_SUFFIX, WRITE)                        \
    ...
    {                                                                        \
        ...
        for (;;) {                                                           \
            ...
            const uint32_t *desc =                                           \
                &vsnd->ram[queue->queue_desc + desc_idx * 4];                \
                                                                             \
        ...
        list_for_each_entry (node, &q, q) {                                  \
            ...
            if (idx == 0) { /* the first descriptor */                       \
                ...
                if (WRITE)                                                   \
                    ...
                    bad_msg_err = stream_id != flush_stream_id ? 1 : 0;      \
                goto early_continue;                                         \
            } else if (idx == cnt - 1) { /* the last descriptor */           \
                if (!WRITE && bad_msg_err == 1) {                            \
                    rv_log_warn("ignore flush stream_id %" PRIu32,           \
                                stream_id);                                 \
                    goto early_continue;                                     \
                }                                                            \
                if (!WRITE)                                                  \
                    rv_log_info("flush stream_id %" PRIu32, stream_id);      \
...
static void virtio_snd_update_status(virtio_snd_state_t *vsnd, uint32_t status)
{
    ...
    bool initialized = vsnd->initialized;
    memset(vsnd, 0, sizeof(*vsnd));
    vsnd->ram = ram;
    vsnd->priv = priv;
    vsnd->initialized = initialized;
                                        ...
                                        uint32_t *plen)
{
    ...
    if (err != paNoError) {
        rv_log_error("Cannot create PortAudio: %s", Pa_GetErrorText(err));
        return;
    }
                                      ...
                                      uint32_t *plen)
{
    ...
    if (err != paNoError) {
        rv_log_error("get error in pcm_start: %s", Pa_GetErrorText(err));
        return;
    }
                                     ...
                                     uint32_t *plen)
{
    ...
    if (err != paNoError) {
        rv_log_error("get error in pcm_stop: %s", Pa_GetErrorText(err));
        return;
    }
                                        ...
                                        virtio_snd_state_t *vsnd)
{
    ...
    if (err != paNoError)
        rv_log_error("get error in pcm_release: %s", Pa_GetErrorText(err));
                                       ...
                                       uint32_t stream_id)
{
    ...
    while (!list_empty(&props->buf_queue_head) && written_bytes < n) {
        ...
        memcpy(out_bytes + written_bytes,
               (const uint8_t *) node->addr + node->pos, len);
                                        ...
                                        uint32_t *plen)
{
    ...
    for (int i = 0; i < VSND_DESC_CNT; i++) {
        ...
        const struct virtq_desc *desc =
            (const struct virtq_desc *) &vsnd->ram[queue->queue_desc +
                                                  desc_idx * 4];
    ...
    const virtio_snd_hdr_t *request =
        (const virtio_snd_hdr_t *) ((uintptr_t) vsnd->ram + vq_desc[0].addr);
    uint32_t type = request->code;
    virtio_snd_hdr_t *response =
        (virtio_snd_hdr_t *) ((uintptr_t) vsnd->ram + vq_desc[1].addr);
    const void *query = (const void *) ((uintptr_t) vsnd->ram + vq_desc[0].addr);
    ...
    case VIRTIO_SND_R_PCM_STOP:
        ...
        break;
    default:
        rv_log_error("%" PRIu32 ": unsupported virtio-snd operation!", type);
                   ...
                   uint32_t *))
{
    ...
    /* Process them */
    uint16_t new_used = ram[queue->queue_used] >> 16; /* virtq_used.idx (le16) */
    ...
    vsnd->ram[queue->queue_used] &= MASK(16); /* Reset low 16 bits to zero */
    vsnd->ram[queue->queue_used] |= ((uint32_t) new_used) << 16; /* len */
...
#define _(reg) VIRTIO_##reg
    switch (addr) {
    case _(MagicValue):
        *value = VIRTIO_MAGIC_NUMBER;
        return true;
    case _(Version):
        *value = VIRTIO_VERSION;
        return true;
    case _(DeviceID):
        *value = VIRTIO_SND_DEV_ID;
        return true;
    case _(VendorID):
        *value = VIRTIO_VENDOR_ID;
        return true;
    case _(DeviceFeatures):
        *value = vsnd->device_features_sel == 0
                     ? VSND_FEATURES_0
    ...
    case _(InterruptStatus):
        *value = vsnd->interrupt_status;
        return true;
    case _(Status):
        *value = vsnd->status;
        return true;
    case _(ConfigGeneration):
        *value = VIRTIO_CONFIG_GENERATE;
        return true;
    default: {
        uint32_t config_idx = addr - _(Config);
        if (config_idx >= sizeof(virtio_snd_config_t) / sizeof(uint32_t)) {
            virtio_snd_set_fail(vsnd);
            return false;
        }

        /* Read configuration from the corresponding register */
        *value = ((uint32_t *) PRIV(vsnd))[config_idx];
        return true;
    }
    }
#undef _
...
#define _(reg) VIRTIO_##reg
    ...
    case _(DriverFeatures):
        vsnd->driver_features_sel == 0 ? (vsnd->driver_features = value) : 0;
        return true;
    ...
    case _(QueueNotify):
        if (value < ARRAY_SIZE(vsnd->queues)) {
            ...
            case VSND_QUEUE_TX:
                ...
                break;
            default:
                rv_log_error("value %" PRIu32 " not supported", value);
                return false;
            }
        } else {
            virtio_snd_set_fail(vsnd);
    ...
    default: {
        ...
        ((uint32_t *) PRIV(vsnd))[config_idx] = value;
        return true;
    }
    }
#undef _
...
uint32_t virtio_snd_read(virtio_snd_state_t *vsnd, uint32_t addr)
{
    ...
    addr = addr >> 2;
    if (!virtio_snd_reg_read(vsnd, addr, &value))
        return 0;
...
{
    addr = addr >> 2;
    virtio_snd_reg_write(vsnd, addr, value);
}

bool virtio_snd_init(virtio_snd_state_t *vsnd)
{
    if (vsnd_dev_cnt >= VSND_DEV_CNT_MAX) {
        rv_log_error("Exceed the number of virtio-snd devices that can be "
                     "allocated.");
        return false;
    }
    ...
    if (pthread_create(&tid, NULL, func, vsnd) != 0) {
        rv_log_error("cannot create TX thread");
        return false;
    }
    /* Initialize PortAudio */
    PaError err = Pa_Initialize();
    if (err != paNoError) {
        rv_log_error("PortAudio error: %s", Pa_GetErrorText(err));
        Pa_Terminate();
        return false;
    }
    vsnd->initialized = true;
...
void virtio_snd_shutdown(virtio_snd_state_t *vsnd)
{
    ...
    Pa_Terminate();
    vsnd->initialized = false;
}

#else

uint32_t virtio_snd_read(virtio_snd_state_t *vsnd UNUSED, uint32_t addr UNUSED)
{
    return 0;
}

void virtio_snd_write(virtio_snd_state_t *vsnd UNUSED,
                      uint32_t addr UNUSED,
                      uint32_t value UNUSED)
{
}

bool virtio_snd_init(virtio_snd_state_t *vsnd UNUSED)
{
    return false;
}

void virtio_snd_shutdown(virtio_snd_state_t *vsnd UNUSED)
{
}
